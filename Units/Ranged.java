...
public class Ranged extends BaseHero {
    ...
    public Ranged(ArrayList<BaseHero> teamList, String name, String role, String icon, int attack, int defence,
                  int[] damage, int health, int speed, int shoots, int x, int y, String team, boolean status) {
    ...
    public void step(ArrayList<BaseHero> heroList) {
        ...
        for (BaseHero hero : heroList) {
            if (hero.role.equals("Farmer")) {
                if (((Farmer) hero).supply) {
                    ...
                    System.out.printf("\nПерсонаж %s %s совершил выстрел в персонажа %s %s нанёс %d урона и потратил" +
                                    " 0 стрел. Осталось стрел %d/%d", name, role, heroList.get((int) target.y).name,
                            heroList.get((int) target.y).role, (int) attackPower, shoots, maxShoots);
        ...
        if (doShoot) {
            if (shoots > 0) {
                heroList.get((int) target.y).getDamage(attackPower);
                System.out.printf("\nПерсонаж %s %s совершил выстрел в персонажа %s %s нанёс %d урона и потратил" +
                                " 1 стрелу. Осталось стрел %d/%d", name, role, heroList.get((int) target.y).name,
                        heroList.get((int) target.y).role, (int) attackPower, --shoots, maxShoots);
            } else {
                System.out.printf("\nПерсонаж %s %s не может больше стрелять. Закончились стрелы %d/%d",
                        name, role, shoots, maxShoots);
            }
        }
    }
